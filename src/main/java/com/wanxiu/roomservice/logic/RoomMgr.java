package com.wanxiu.roomservice.logic;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.google.gson.Gson;
import com.wanxiu.roomservice.common.Config;
import com.wanxiu.roomservice.pojo.Audience;
import com.wanxiu.roomservice.pojo.Pusher;
import com.wanxiu.roomservice.pojo.Response.GetStreamStatusRsp;
import com.wanxiu.roomservice.pojo.Room;
import com.wanxiu.roomservice.utils.Utils;
...
public class RoomMgr implements InitializingBean {
    ...
    private ConcurrentHashMap<String, ConcurrentHashMap<String,Room>> roomIdByTypeMap=new ConcurrentHashMap<>();
     ...
     * 获取房间列表根据房间类型
     */
    public ArrayList<Room> getListWithRoomType(int cnt, int index, boolean withPushers, int type,String roomType) {

        ArrayList<Room> resultList = new ArrayList<>();
        if(!roomIdByTypeMap.containsKey(roomType)){
            return resultList;
        }
        ConcurrentHashMap<String, Room> orgMap = roomIdByTypeMap.get(roomType);
        int cursor = 0;
        int roomCnt = 0;
        //遍历
        for (Room value : orgMap.values()) {
    ...
    public void creatRoom(String roomID, String roomInfo, int type) {
        ...
        log.info("我的日志1" + new Gson().toJson(getActuralMap(type)));
    }
    ...
    public void addAudience(String roomID, String userID, String userInfo, int type) {
        ...
        if (room != null) {
            ...
            for (Audience a: room.getAudiences()) {
                if(a.getUserID().equals(userID)){
                   room.delAudience(userID);
                   room = liveRoomMap.get(roomID);
                }
            }
    ...
    public ArrayList<Audience> getAudiencesByUserId(String roomID, int type,String userID) {
        // 只有直播房间有观众
        if (type == LIVE_ROOM) {
            Room room = liveRoomMap.get(roomID);
            if (room != null){
                log.info("用户编号"+userID);
                log.info("开始测试获取房间内观众");
                List<Audience> roomAudiences=room.getAudiences();
                for(Audience a:roomAudiences){
                    log.info("遍历房间");
                    if(a.getUserID().equals(userID)){
                        log.info("发现房间内存在重复信息");
                        room.delAudience(userID);
                    }
                }
                }
    ...
    public void setRoomWidthType(String roomID,String roomType,int type){
        Room room=getRoom(roomID,type);
        if(roomIdByTypeMap.containsKey(roomType)){
            ConcurrentHashMap<String,Room> m = roomIdByTypeMap.get(roomType);
            m.put(roomID,room);
            roomIdByTypeMap.put(roomType,m);
        }else{
            ConcurrentHashMap<String,Room> m=new ConcurrentHashMap<>();
            m.put(roomID,room);
            roomIdByTypeMap.put(roomType,m);
        }
    }

    public void delRoomType(String roomID,String roomType){
        if(roomIdByTypeMap.containsKey(roomType)){
            ConcurrentHashMap<String,Room> m =roomIdByTypeMap.get(roomType);
            m.remove(roomID);
            roomIdByTypeMap.put(roomType,m);
