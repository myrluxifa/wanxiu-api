...
import com.wanxiu.roomservice.pojo.StreamIDS;
...
public class RoomServiceImpl implements RoomService {
    ...
    public Room getPushers(String userID, String token, GetPushersReq req, int type) {
        ...
        log.info("我的日志5" + new Gson().toJson(rsp));
    ...
    public BaseRsp setRoomType(String userID, String token,SetRoomTypeReq req,int type){
        BaseRsp rsp = new BaseRsp();
        String roomID = req.getRoomID();
        ...
        roomMgr.setRoomWidthType(roomID,roomType,type);
        return rsp;
    }

    public BaseRsp delRoomType(String userID, String token,SetRoomTypeReq req,int type){
        BaseRsp rsp = new BaseRsp();
        String roomID = req.getRoomID();
        String roomType=req.getRoomType();
        if (userID == null || token == null || roomID == null) {
        ...
        roomMgr.delRoomType(roomID,roomType);
        return rsp;
    }

    @Override
    public GetRoomListRsp getRoomListRoomType(String userID, String token, GetRoomListRoomTypeReq req, int type) {
        GetRoomListRsp rsp = new GetRoomListRsp();
        if (userID == null || token == null) {
            rsp.setCode(2);
            rsp.setMessage("请求失败，缺少参数");
        ...
        rsp.setList(roomMgr.getListWithRoomType(req.getCnt(), req.getIndex(), true,type,req.getRoomType()));
        return rsp;
    }
}
